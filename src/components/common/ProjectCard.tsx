      ...
      <div className="flex flex-col flex-grow p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="text-xs px-2 py-1 bg-gray-100 dark:bg-black text-gray-700 dark:text-gray-300 rounded-full"
            >
              {tag}
            </span>
          ))}
          {project.tags.length > 3 && (
            <span className="text-xs px-2 py-1 bg-gray-100 dark:bg-black text-gray-700 dark:text-gray-300 rounded-full">
              +{project.tags.length - 3}
            </span>
          )}
        </div>
